...
```cpp
/*796.子矩阵的和
输入一个 n nn 行 m mm 列的整数矩阵，再输入 q qq 个询问，每个询问包含四个整数 x 1 , y 1 , x 2 , y 2 ， x1,y1,x2,y2，x1,y1,x2,y2，表示一个子矩阵的左上角坐标和右下角坐标。
对于每个询问输出子矩阵中所有数的和。
输入格式
第一行包含三个整数 n ， m ， q n，m，qn，m，q。
接下来 n nn 行，每行包含 m mm 个整数，表示整数矩阵。
接下来 q qq 行，每行包含四个整数 x 1 , y 1 , x 2 , y 2 ， x1,y1,x2,y2，x1,y1,x2,y2，表示一组询问。
输出格式
共 q qq 行，每行输出一个询问的结果。
数据范围
